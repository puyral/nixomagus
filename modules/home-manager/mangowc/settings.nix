{lib,config, ...}: with lib; let
  ...
  tags = map toString [1 2 3 4 5 6 7 8 9];
  id = x: x;

  mkModKey = strings.concatImapStringsSep "+";
  mkArgs = strings.concatMapAttrsStringSep ",";
  ...
  movetotag = mkBindTag [M S] id (t: ["tagsilent" t "0"]);

  stacktags = mkBindTag [M C] id (t: ["toggleview" t "0"]);
  ...
  settings = [(builtins.readFile ./config.conf) viewtag movetotag stacktags swapwin switchfocus];
 in
   {
   config = mkIf cfg.enable { 
      settings = strings.concatStringsSep "\n" settings;
   };
   }